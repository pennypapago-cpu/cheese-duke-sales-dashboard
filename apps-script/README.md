...
- **商品金額**：實際的「官網訂單」**沒有整行金額欄位**，商品區只有「商品成本／商品原價／商品結帳價」三個單價欄。因此商品排行的銷售額一律用 **商品結帳價 × 數量** 計算。結帳價已是折後價（見「結帳價類型」欄），不再另外扣「商品折扣金額」。若之後表上新增了「商品合計」欄，腳本會自動改用該欄。
- **商品金額 vs 營收**：商品排行的金額加總不必然等於 KPI 的營收 —— 營收取自訂單合計（含運費、全單折扣、購物金折抵），商品金額只是品項層級的加總。所以排行的「銷售額佔比」分母用的是商品金額總計，不是營收。
- **欄位定位**：先比對標題列名稱，找不到才退回固定欄序（訂單合計 G、訂單來源 S、訂單日期 U、商品貨號 BW、商品結帳價 CB、數量 CE…），所以中間插欄不會壞掉。
